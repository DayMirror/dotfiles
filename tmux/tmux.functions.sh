...
EOF
  ...
  while IFS= read -r line; do
    ...
    if _is_enabled "$tmux_conf_new_pane_reconnect_ssh"; then
      right=$(printf '%s' "$line" | cut -c"$column"- | awk -F'\"' 'BEGIN { OFS = FS } { for (i = 1; i <= NF; i+=2) { gsub(/#{.+}/, "\"&\"", $i) } print }' | sed -E -e '/_split_window/!s/"/\\"/g' -e 's/split-window([^;]*)/run-shell "cat ~\/\.tmux\.conf | sh -s _split_window #{pane_tty}\1"/g')
    else
      right=$(printf '%s' "$line" | cut -c"$column"- | awk -F'\"' 'BEGIN { OFS = FS } { for (i = 1; i <= NF; i+=2) { gsub(/#{.+}/, "\"&\"", $i) } print }' | sed -E -e 's/\\"/"/g' -e 's/run-shell "cat ~\/\.tmux\.conf \| sh -s _split_window #\{pane_tty\}([^;]*)"/split-window\1/g' -e 's/#\{.+\}/\"&\"/g')
    fi
    eval "tmux $left $right" 2>/dev/null || true
  done  << EOF
$(tmux list-keys 2>/dev/null | grep -E 'split(-|_)window')
...
_apply_theme() {
  ...
  tmux_conf_theme_terminal_title=$(echo "$tmux_conf_theme_terminal_title" | sed \
    -e 's%#{circled_window_index}%#(cat ~/.tmux/tmux.functions.sh | sh -s _circled #I)%g' \
    -e 's%#{circled_session_name}%#(cat ~/.tmux/tmux.functions.sh | sh -s _circled #S)%g' \
    -e 's%#{username}%#(cat ~/.tmux/tmux.functions.sh | sh -s _username #{pane_tty} false #D)%g' \
    -e 's%#{hostname}%#(cat ~/.tmux/tmux.functions.sh | sh -s _hostname #{pane_tty} false #D)%g' \
    -e 's%#{username_ssh}%#(cat ~/.tmux/tmux.functions.sh | sh -s _username #{pane_tty} true #D)%g' \
    -e 's%#{hostname_ssh}%#(cat ~/.tmux/tmux.functions.sh | sh -s _hostname #{pane_tty} true #D)%g')
  ...
  tmux_conf_theme_status_left=$(echo "$tmux_conf_theme_status_left" | sed \
    -e "s%#{root}%#[fg=$tmux_conf_theme_root_fg]#[bg=$tmux_conf_theme_root_bg]#[$tmux_conf_theme_root_attr]#(cat ~/.tmux/tmux.functions.sh | sh -s _root #{pane_tty} #D)#[inherit]%g")
...
EOF
  ...
  case "$status_left $status_right" in
    *'#{battery_status}'*|*'#{battery_bar}'*|*'#{battery_hbar}'*|*'#{battery_vbar}'*|*'#{battery_percentage}'*)
      ...
      status_right="#(cat ~/.tmux/tmux.functions.sh | sh -s _battery)$status_right"
      ;;
  esac
  ...
  case "$status_left $status_right" in
    *'#{uptime_d}'*|*'#{uptime_h}'*|*'#{uptime_m}'*|*'#{uptime_s}'*)
      ...
      status_right=$(echo "$status_right" | sed -E \
        ...
        -e 's/#\{(\?)?uptime_m/#\{\1@uptime_m/g' \
        -e 's/#\{(\?)?uptime_s/#\{\1@uptime_s/g')
      status_right="#(cat ~/.tmux/tmux.functions.sh | sh -s _uptime)$status_right"
      ;;
  esac
  ...
  case "$status_left $status_right" in
    *'#{loadavg}'*)
      ...
      status_right=$(echo "$status_right" | sed -E \
        -e 's/#\{(\?)?loadavg/#\{\1@loadavg/g')
      status_right="#(cat ~/.tmux/tmux.functions.sh | sh -s _loadavg)$status_right"
      ;;
  esac

  status_left=$(echo "$status_left" | sed 's%#{circled_session_name}%#(cat ~/.tmux/tmux.functions.sh | sh -s _circled #S)%g')
  status_right=$(echo "$status_right" | sed 's%#{circled_session_name}%#(cat ~/.tmux/tmux.functions.sh | sh -s _circled #S)%g')
